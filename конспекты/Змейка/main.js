...
function getRandomCoordinates(mouse = false) {
    const coef = mouse ? 1 : 3;
    const x = Math.round(Math.random() * (FIELD_SIZE - coef) + coef);
...
function move() {
    ...
    const obj = Object.values(directionsDict).find(
...
document.addEventListener("keydown", (e) => {
    const newDirection = directionsDict[e.key];
    if (!newDirection) return;
    const currentType = Object.values(directionsDict).find(
        (el) => el.value === direction
    ).type;
